      ...
      <div className="flex-1 bg-white rounded-xl border border-gray-100 shadow-sm flex flex-col overflow-hidden">
        ...
        <div className="flex-1 overflow-x-auto">
          <table className="w-full text-left text-sm whitespace-nowrap">
            ...
            <span className="material-symbols-outlined text-green-600 text-2xl">alarm_on</span>
          </div>
          <div>
        ...
        <div className="bg-white p-5 rounded-xl border border-gray-100 shadow-sm flex items-center gap-4">
          <div className="w-12 h-12 rounded-full bg-yellow-50 flex items-center justify-center border border-yellow-100">
            <span className="material-symbols-outlined text-yellow-600 text-2xl">history</span>
          </div>
          <div>
        ...
        <div className="bg-white p-5 rounded-xl border border-gray-100 shadow-sm flex items-center gap-4">
          <div className="w-12 h-12 rounded-full bg-blue-50 flex items-center justify-center border border-blue-100">
            <span className="material-symbols-outlined text-blue-600 text-2xl">co_present</span>
          </div>
          <div>
            <p className="text-sm font-medium text-gray-500">Currently Active</p>
            <h3 className="text-xl font-bold text-gray-800">75</h3>
          </div>
        </div>
      </div>
          ...
          <table className="w-full text-left text-sm whitespace-nowrap">
            <thead className="bg-gray-50 text-gray-500 font-semibold border-b border-gray-100 uppercase text-[11px] tracking-wider">
              <tr>
                ...
                <th className="px-6 py-4">Arrival Remark</th>
            ...
            <tbody className="divide-y divide-gray-50">
              {presentEmployees.map((emp) => (
                <tr key={emp.id} className="hover:bg-gray-50/50 transition">
                  ...
                  {/* Department */}
                  <td className="px-6 py-4">
                    <span className="text-gray-600 font-medium">{emp.department}</span>
                  </td>

                  {/* Check-in Time */}
                  <td className="px-6 py-4">
                    <div className="flex items-center gap-2">
                      <span className="material-symbols-outlined text-[16px] text-gray-400">login</span>
                      <span className="font-bold text-gray-800">{emp.checkIn}</span>
                    </div>
                  </td>

                  {/* Arrival Remark (On Time vs Late) */}
                  <td className="px-6 py-4">
                    <span className={`px-3 py-1 rounded-full text-[11px] font-bold uppercase tracking-wider border flex items-center w-max gap-1 ${
                      emp.remark === 'On Time' ? 'bg-green-50 text-green-700 border-green-200' : 'bg-yellow-50 text-yellow-700 border-yellow-200'
                    }`}>
                      <span className="material-symbols-outlined text-[14px]">
                        {emp.remark === 'On Time' ? 'task_alt' : 'schedule_warning'}
                      </span>
                      {emp.remark}
                    </span>
                  </td>
                  ...
                  {/* Action */}
                  <td className="px-6 py-4 text-center">
                    <button className="text-blue-600 hover:bg-blue-50 px-3 py-1.5 rounded-lg border border-transparent hover:border-blue-200 transition font-medium text-xs">
                      View Log
                    </button>
                  </td>
